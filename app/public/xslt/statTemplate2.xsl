		...
		<html>
			<body>
				...
				<svg width="300" height="300" style= "background-color: lightgray;border-radius: 50%;">
					...
					<xsl:variable name="decalage">0</xsl:variable>
					<xsl:for-each select="./nombres/nombre">
						<xsl:variable name="nombre"  select="."/>

						<xsl:variable name="portion" select="$nombre div $total *942"/>
						<!-- <xsl:variable name="portion" select="{$nombre/$total*942}"></xsl:variable> -->
						<circle r="150" cx="150" cy="150" class="pie" fill-opacity="0" style='stroke:lightgreen;stroke-width: 300;stroke-dasharray: 0,{$decalage},{$portion},942;'></circle>

						<!-- <xsl:variable name="decalage" select="$decalage + $portion"/> -->
					</xsl:for-each>
				</svg>
				<br/>
				<h2></h2>
